...
const BulkOrderManagement: React.FC = () => {
  ...
  const fetchBulkOrders = async () => {
    try {
      ...
      const formattedOrders: BulkOrder[] =
        orders?.map((order) => ({
          id: order.id,
  ...
  const totalRevenue = bulkOrders.reduce(
    (sum, order) => sum + order.total_price,
    0,
  );
  const averageOrderValue =
    totalBulkOrders > 0 ? totalRevenue / totalBulkOrders : 0;
                  ...
                  <div className="flex justify-between items-start">
                    <div className="space-y-2">
                      ...
                      <div className="text-sm">
                        Total: {order.total_quantity} items - RM{" "}
                    ...
                    <div className="space-y-1">
                      {order.products.map((product, index) => (
                        ...
                        >
                          ...
                          <span>
                            RM {(product.quantity * product.price).toFixed(2)}
                          </span>
                        </div>
                      ))}
                    </div>
