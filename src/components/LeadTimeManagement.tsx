...
const LeadTimeManagement: React.FC = () => {
      ...
      orders?.forEach(order => {
        order.order_items?.forEach((item: any) => {
          if (item.products?.fulfillment_lead_time_days) {
            ...
            const fulfillmentDate = new Date(order.estimated_fulfillment_date || orderDate);
            const leadTime = item.products.fulfillment_lead_time_days;
            const productionStartDate = new Date(fulfillmentDate);
            productionStartDate.setDate(productionStartDate.getDate() - leadTime);
            
            const daysRemaining = Math.ceil((productionStartDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
  ...
  const warningDeadlines = deadlines.filter(d => d.status === 'warning').length;
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-red-600">{urgentDeadlines}</div>
            <p className="text-sm text-muted-foreground">Urgent Deadlines</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <div className="text-2xl font-bold text-yellow-600">{warningDeadlines}</div>
                ...
                <div key={product.id} className="flex items-center justify-between p-4 border rounded">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Current Lead Time: {product.fulfillment_lead_time_days || 'Not Set'} days
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {editingProduct === product.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <Button
                          size="sm"
                          onClick={() => updateLeadTime(product.id, parseInt(leadTimeInput) || 0)}
                        >
                          Save
                        </Button>
                        <Button
              ...
              {deadlines.map((deadline, index) => (
                <div key={index} className="flex items-center justify-between p-4 border rounded">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-muted-foreground">
                      Order Date: {new Date(deadline.order_date).toLocaleDateString()}
                    </div>
                    <div className="text-sm text-muted-foreground">
